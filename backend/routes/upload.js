...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedTypes.includes(file.mimetype) || allowedExtensions.includes(fileExtension)) {
    ...
    cb(new Error('Invalid file type. Only text, markdown, PDF, and script files are allowed.'), false);
  }
};
...
router.post('/upload-scripts', auth, upload.array('scripts', 10), async (req, res) => {
  try {
